...
if (!empty($_GPC['appid'])) {
    ...
    if ($appid == 'wx570bc396a51b8ff8') {
        ...
        $id =
            pdo_fetchcolumn("SELECT acid FROM " . tablename('account_wechats') . " WHERE `key` = :appid", array(':appid' => $appid));
    }
}
if (empty($id)) {
    $id = intval($_GPC['id']);
...
class WeEngine
{
    ...
    private function booking($message)
    {
        ...
        if (empty($default_groupid)) {
            $default_groupid =
                pdo_fetchcolumn('SELECT groupid FROM ' . tablename('mc_groups') . ' WHERE uniacid = :uniacid AND isdefault = 1', array(':uniacid' => $_W['uniacid']));
    ...
    private function analyzeSubscribe(&$message)
    {
        ...
        if (!empty($message['scene'])) {
            ...
            $qr =
                pdo_fetch("SELECT `id`, `keyword` FROM " . tablename('qrcode') . " WHERE {$scene_condition} AND `uniacid` = '{$_W['uniacid']}'");
            if (!empty($qr)) {
    ...
    private function analyzeQR(&$message)
    {
        ...
        if (empty($qr) && !empty($message['ticket'])) {
            ...
            if (!empty($ticket)) {
                $qr =
                    pdo_fetchall("SELECT `id`, `keyword` FROM " . tablename('qrcode') . " WHERE `uniacid` = '{$_W['uniacid']}' AND ticket = '{$ticket}'");
                if (!empty($qr)) {
                    if (count($qr) != 1) {
                        $qr = array();
                    } else {
        ...
        if (!empty($message['picurl'])) {
            ...
            if (!empty($response)) {
                ...
                if (!empty($event['keyword'])) {
                    ...
                    $event =
                        pdo_fetch("SELECT keyword, type FROM " . tablename('menu_event') . " WHERE openid = '{$message['from']}'");
                }
                if (!empty($event)) {
