...
function slashadmin_login_page() {
	...
	$headers = ! empty( $image ) ? get_headers( $image ) : '';
	list( $width, $height ) = ! empty( $image ) && ( strpos( $headers[0], '404' ) === false ) && ini_get( 'allow_url_fopen' ) ? getimagesize( $image ) : array(
		'',
		''
	); // Get the uploaded image's width and height
...
if ( slash_admin( 'login_links' ) == 1 ) {
	...
	add_filter( 'login_headertitle', 'slashadmin_login_page_url_title' );
}
