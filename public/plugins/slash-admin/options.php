...
class Slash_Admin_Options {
	...
	public function add_pages() {
		...
		$admin_page = add_management_page( __( 'Slash Admin Options', 'slash-admin' ), __( 'Slash Admin', 'slash-admin' ), 'manage_options', 'slashadmin-options', array(
			&$this,
			'display_page'
		) );
	...
	public function create_setting( $args = array() ) {
		...
		add_settings_field( $id, $title, array(
			$this,
			'display_setting'
		), 'slashadmin-options', $section, $field_args );
	}
	...
	<h1 class="slash-admin-header">' . __( 'Slash Admin Options', 'slash-admin' ) . '</h1>';
		...
		echo '</div>
		<p class="submit"><input name="Submit" type="submit" class="button-primary" value="' . __( 'Save Changes', 'slash-admin' ) . '" /></p>
	...
	public function display_setting( $args = array() ) {
		...
		switch ( $type ) {
			...
			case 'checkbox':
				...
				echo '<input class="checkbox' . $field_class . '" type="checkbox" id="' . $id . '" name="slashadmin_options[' . $id . ']" value="1" ' . checked( $options[ $id ], 1, false ) . ' /> <label for="' . $id . '">' . $desc . '</label>';
			...
			case 'multiple':
				...
				foreach ( $choices as $value => $label ) {
					$selected = ( is_array( $options[ $id ] ) && in_array( $value, $options[ $id ] ) ) ? 'selected="selected"' : '';
			...
			case 'upload': // Media uploader (see: http://www.justinwhall.com/multiple-upload-inputs-in-a-wordpress-theme-options-page/)
				echo '<input id="' . $id . '" class="upload-url' . $field_class . '" type="text" name="slashadmin_options[' . $id . ']" value="' . esc_attr( $options[ $id ] ) . '" /><input id="st_upload_button" class="st_upload_button" type="button" name="upload_button" value="' . __( 'Upload', 'slash-admin' ) . '" />';
			...
			case 'radio':
				$i = 0;
				foreach ( $choices as $value => $label ) {
					echo '<input class="radio' . $field_class . '" type="radio" name="slashadmin_options[' . $id . ']" id="' . $id . $i . '" value="' . esc_attr( $value ) . '" ' . checked( $options[ $id ], $value, false ) . '> <label for="' . $id . $i . '">' . $label . '</label>';
	...
	public function get_settings() {
		...
		$this->settings['default_opensans_remove'] = array(
			...
			'desc'    => __( 'Select how you should deal with the default Open Sans fonts that WordPress uses. If you have issues with how the font is displayed on logged-in users (it usually happens with non-latin subsets), try to disable it and then re-enter them properly below.', 'slash-admin' ),
			'type'    => 'select',
			'std'     => 'default',
			'choices' => array(
		...
		$this->settings['custom_fonts']            = array(
			...
			'desc'    => __( 'Insert your Google Web Fonts here. You can declare more than one fonts, styles and language subsets. Keep in mind that you must only fill in the part after the "http://fonts.googleapis.com/css?family=" like for example <code>Open+Sans:400,300&subset=latin,greek</code>. View more information about the font\'s structure, subsets and weights at the <a href="https://developers.google.com/fonts/docs/getting_started#Overview" target="_blank">Google Fonts help page</a>. If your theme has it\'s own mechanism for embedding Google Fonts or if you prefer to do it manually, just leave it blank.', 'slash-admin' ),
			'type'    => 'text',
			'std'     => ''
		);
		...
		$this->settings['cookielaw_header']   = array( // Cookie law
			...
			'type'    => 'heading',
			'para'    => __( 'Show how your website complies with the EU Cookie Law. It displays a message at the bottom or the top of the page asking for the user\'s consent. Read more about <a href="http://ec.europa.eu/ipg/basics/legal/cookies/index_en.htm" target="_blank">EU legislation on cookies</a>', 'slash-admin' )
		);
		...
		$this->settings['cookielaw_message']  = array(
			...
			'desc'    => __( 'Change the default message. If you use WPML and you want a different message per language, you should enable "Get message from page" option below.', 'slash-admin' ),
			'type'    => 'text',
		...
		$this->settings['cookielaw_accept']   = array(
			...
			'desc'    => __( 'Change the "Accept" button text. If you use WPML and you want a different wording per language, go to your terms page (as set via "Read more page" option below) and fill in your new value in the custom field "slash_accept".', 'slash-admin' ),
			'type'    => 'text',
		...
		$this->settings['cookielaw_url'] = array(
			...
			'desc'    => __( 'The page with the details about how your website uses cookies. If you don\'t select one, then no "read more" link will appear.', 'slash-admin' ),
			'type'    => 'select',
			'std'     => 'none',
		...
		$this->settings['cookielaw_pagedata'] = array(
			...
			'desc'    => __( 'Retrieve the box message form from your "Read more" page\'s excerpt instead of the plugin\'s message field. You need to enable excerpt support to pages from the "Miscellaneous" section below. Check this option if you use WPML and you want to have different consent messages per language.', 'slash-admin' ),
			'type'    => 'checkbox',
			'std'     => 0
		);
		...
		$this->settings['loading_img']     = array(
			...
			'desc'    => __( 'If you use a site icon (via <strong>Appearance &rarr; Customize &rarr; Site Identity &rarr; Site Icon</strong>) it will be used as the loader\'s image. In that case, just leave this field empty. If you want to display a different image as your loader, upload it here. The image should be square and no more than 160x160 pixels.', 'slash-admin' ) . '</span>',
			'std'     => '',
			'type'    => 'upload',
			'section' => 'appearance'
		);
		...
		$this->settings['loading_bg_color']      = array(
			...
			'desc'    => __( 'The loader\'s background color. You can use hex (e.g. <code>#FFFFFF</code>) or rgb/rgba (e.g. <code>rgba(255,255,255,0.8)</code>).', 'slash-admin' ),
			'type'    => 'text',
			'std'     => '#FFFFFF'
		);
		...
		$this->settings['loading_hide']          = array(
			...
			'desc'    => __( 'The text to be displayed if the spinner takes too long. When it gets clicked, it hides the spinner.', 'slash-admin' ),
			'type'    => 'text',
		...
		$this->settings['loading_location']      = array(
			...
			'desc'    => __( 'If checked, it will show the loader only on the homepage. Leave it unchecked to show it on all your website\'s pages. This option gets ignored if you insert the function manually.', 'slash-admin' ),
			'type'    => 'checkbox',
			'std'     => 0
		);
		...
		$this->settings['loading_manual'] = array(
			...
			'desc'    => __( 'Normally, the function is hooked to the footer via the <code>wp_footer</code> hook. For better results, you can disable that hook and manually add the loader to your template using <code>&lt;?php do_action(\'slash_admin_loader\'); ?&gt;</code>, prefferably right after your <code>&lt;body&gt;</code> opening.', 'slash-admin' ),
			'type'    => 'checkbox',
			'std'     => 0
		);
		...
		$this->settings['remove_category'] = array( // Remove the category from the title
			'section' => 'appearance',
			'title'   => __( 'Remove "Category:" from archives', 'slash-admin' ),
			'desc'    => __( 'Gets rid of the word "Category:" in front of the Archive title.', 'slash-admin' ),
			'type'    => 'checkbox',
			'std'     => 0
		);
		...
		$this->settings['oldie_warning'] = array( // Hide update notices for all but Admins
			...
			'desc'    => __( 'If the visitor uses an old version of Internet Explorer (aka IE9 or older) display a message at the top of the page warning him that he is using an outdated browser and that he should update to a newer one.', 'slash-admin' ),
			'type'    => 'checkbox',
			'std'     => 0
		);
		...
		$this->settings['favicon']        = array( // Favicon
			...
			'desc'    => '<span class="alert">' . __( 'Since WordPress 4.3, you can use a native Customizer option to upload your favicon (Under Appearance &rarr; Customize &rarr; Site Identity &rarr; Site Icon). This option here is kept for legacy reasons, but you should prefer the WordPress\' default.', 'slash-admin' ) . '</span>',
			'std'     => '',
			'type'    => 'upload',
			'section' => 'appearance'
		);
		...
		$this->settings['analytics']        = array(
			...
			'desc'    => __( 'Add code for Google Analytics (it will be inserted before the <code>&lt;/head&gt;</code> tag).', 'slash-admin' ),
			'std'     => '',
			'type'    => 'textarea',
			'section' => 'administration'
		);
		...
		$this->settings['limit_revisions']        = array( // Limit Revisions
			...
			'desc'    => __( 'Limit the number of revisions that WordPress keeps for each post type. By default, an infinite number of revisions are stored if a post type supports revisions. Keep in mind that if you restrict this number, WordPress will purge the older revisions only after the post is updated.', 'slash-admin' ),
			'type'    => 'select',
			'std'     => 'default',
			'choices' => array(
		...
		$this->settings['prevent_editing'] = array( // Prevent updates on old posts
			...
			'desc'    => __( 'Block post updates and deletion if the post is older than a specific period of time. Applies only to editors (admins can still edit the post as usual).', 'slash-admin' ),
			'type'    => 'select',
			'std'     => 'default',
			'choices' => array(
		...
		$this->settings['jetpack_development_mode'] = array(
			...
			'desc'    => __( 'With Development Mode, features that do not require a connection to WordPress.com servers can be activated on a localhost WordPress installation for testing. Dont\' forget to un-check it back when you go live.', 'slash-admin' ),
			'type'    => 'checkbox',
		...
		$this->settings['maintenance_mode']        = array(
			...
			'desc'    => __( 'If checked, non-Admins will not be able to acess the WordPress backend and they will see the following message instead. Admins can always login as usual. Dont\' forget to un-check it back when you are done.', 'slash-admin' ),
			'type'    => 'checkbox',
			'std'     => 0 // Set to 1 to be checked by default, 0 to be unchecked by default.
		);
		$this->settings['maintenance_mode_msg']    = array(
			'title'   => __( 'Maintenance message', 'slash-admin' ),
			'desc'    => __( 'The message to be displayed if an non-Admin tries to login while in maintenance mode.', 'slash-admin' ),
			'std'     => sprintf( __( 'Site undergoing maintainance. Content uploading and editing will not be available for a while. If you have a question, please contact the website\'s <a href=\'mailto:%s\'>Administrator</a>.', 'slash-admin' ), get_option( 'admin_email' ) ),
			'type'    => 'textarea',
			'section' => 'administration'
		);
		...
		$this->settings['logo'] = array( // Logo
			...
			'desc'    => __( 'Upload the logo to appear at the login screen of your website. The plugin will try to automatically adjust the width/height but the suggested dimensions are <strong>320x80 pixels</strong>.', 'slash-admin' ),
			'std'     => '',
			'type'    => 'upload',
			'section' => 'frontend_usability'
		);
		...
		$this->settings['login_links'] = array(
			...
			'desc'    => __( 'Fix the links at the wordpress login screen in order to link to your website instead of wordpress.org.', 'slash-admin' ),
			'type'    => 'checkbox',
		...
		$this->settings['homepage_redirect'] = array( // Redirect to the homepage after login
			...
			'desc'    => __( 'After log-in, redirect users to the website\'s homepage instead of their profile page.', 'slash-admin' ),
			'type'    => 'checkbox',
			'std'     => 0
		);
		...
		$this->settings['disable_adminbar'] = array( // Disable Admin Bar for non Admins.
			...
			'desc'    => __( 'Disable the Admin Bar for all users except Administrators (applies only to the front-end).', 'slash-admin' ),
			'type'    => 'checkbox',
			'std'     => 0
		);
		$this->settings['login_css']        = array(
			...
			'desc'    => __( 'Your custom styles for the login screen (quick referrence for tags you might want to use: <code>body.login.login-action-login, div#login, div#login h1 a, form#loginform, input#user_login.input, input#user_pass.input, input#wp-submit, p#nav, p#backtoblog</code>).', 'slash-admin' ),
			'type'    => 'textarea'
		);
		...
		$this->settings['exclude_custom']   = array(
			...
			'desc'    => __( 'Exclude other options such as custom post types or even the Dashboard. To do so, add the names separated with commas and no blank spaces like for example <code>Dashboard,Pages,Media,Posts</code> etc. You can exclude <strong>up to 5 additional items</strong>.', 'slash-admin' ),
			'type'    => 'text',
			'std'     => ''
		);
		...
		$this->settings['hide_update_notices'] = array( // Hide update notices for all but Admins
			...
			'desc'    => __( 'Hide the notices for updating Wordpress and other plugins for all users except from Admins', 'slash-admin' ),
			'type'    => 'checkbox',
			'std'     => 0
		);
		...
		$this->settings['hide_specific_pages'] = array(
			...
			'desc'    => __( 'Hide specific pages for non admins. Press "Ctrl/Cmd+Click" to select/deselect multiple pages.', 'slash-admin' ),
			'type'    => 'multiple',
		...
		$this->settings['editors_allow_background']           = array(
			...
			'desc'    => __( 'Allow Editors to change the website\'s background (check this only if your theme has support for WordPress\' <a href="http://codex.wordpress.org/Function_Reference/add_theme_support#Custom_Background" target="_blank">Custom Background</a> feature).', 'slash-admin' ),
			'type'    => 'checkbox',
			'std'     => 0
		);
		$this->settings['editors_gravityforms']               = array(
			...
			'desc'    => __( 'Allow Editors to view entries submitted via Gravity Forms (requires Gravity Forms plugin to be installed, obviously).', 'slash-admin' ),
			'type'    => 'checkbox',
			'std'     => 0
		);
		$this->settings['editors_gravityforms_remove_button'] = array(
			...
			'desc'    => __( ' Remove Gravity Forms\' "Add Form" button from all WYSIWYG editors (requires Gravity Forms plugin to be installed, obviously).', 'slash-admin' ),
			'type'    => 'checkbox',
			'std'     => 0
		);
		...
		$this->settings['admin_logo']         = array( // Logo
			...
			'desc'    => __( 'Replace the WordPress logo at the top left of the screen. Suggested image dimension is <strong>92x92 pixels</strong>.', 'slash-admin' ),
			'std'     => '',
			'type'    => 'upload',
			'section' => 'white_label'
		);
		...
		$this->settings['dashboard_welcome_header'] = array(
			...
			'type'    => 'heading',
			'para'    => __( 'Change the Dashboard welcome message. Removes the default welcome message at the Dashboard and replaces it with your own.', 'slash-admin' )
		);
		$this->settings['dashboard_welcome']        = array(
			'section' => 'white_label',
			'title'   => __( 'Your welcome message', 'slash-admin' ),
			'desc'    => __( 'Add the content of your custom welcome message (HTML markup is allowed).', 'slash-admin' ),
			'type'    => 'textarea'
		);
		...
		$this->settings['dashboard_widget_header']  = array(
			...
			'type'    => 'heading',
			'para'    => __( 'Add a Dashboard Widget to provide general or commercial information to your clients (for example: your contact info or links to support documentation).', 'slash-admin' )
		);
		...
		$this->settings['admin_css']        = array(
			...
			'desc'    => __( 'The styles that you put here affect only the Admin and they don\'t get called at the frontend. Quick referrence for tags you might want to use: <code>div#slashadmin_dashboard_widget</code> for the custom widget, <code>div#welcome-panel</code> for the welcome panel.', 'slash-admin' ),
			'type'    => 'textarea'
		);
		...
		$this->settings['remove_embeds'] = array(
			...
			'desc'    => __( 'If you don\'t use embedded content you can completely disable oEmbeds or load them only on specific pages.', 'slash-admin' ),
			'type'    => 'select',
			'std'     => 'default',
			'choices' => array(
		...
		$this->settings['dns_prefetch']           = array(
			...
			'desc'    => __( 'DNS prefetching notifies the client that there are assets we\'ll need later from a specific URL (outside our website\'s domain) so the browser can resolve the DNS as quickly as possible (<a href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Controlling_DNS_prefetching" target="_blank">read more</a>). Enter here the  URLs to be prefetched (each URL in separate line). Here\'s some examples of sources that can be prefetched for an easy copy/paste: 
					...
					<code>//maxcdn.bootstrapcdn.com</code> (MaxCDN, used by Font Awesome)', 'slash-admin' ),
			'type'    => 'textarea'
		);
		...
		$this->settings['prefetch_prerender_header'] = array(
			...
			'type'    => 'heading',
			'para'    => __( 'Link prefetching is a browser mechanism, which utilizes browser idle time to download or prefetch documents that the user might visit in the near future. A web page provides a set of prefetching hints to the browser, and after the browser is finished loading the page, it begins silently prefetching specified documents and stores them in its cache. When the user visits one of the prefetched documents, it can be served up quickly out of the browser\'s cache. Prerendering downloads and renders the entire page and hides it from the user until it is requested, therefore, it should be used with caution.', 'slash-admin' )
		);
		...
		$this->settings['shortcode_phone'] = array( // Hide update notices for all but Admins
			...
			'desc'    => __( 'Show a telephone number in a way that it is clickable. When clicked, if you are on a mobile device it opens the phone\'s dialer and if you are on a desktop computer it prompts to make a call via a related program (e.g. Skype). Here are some usage examples: <code>[slash_phone number="999999"]</code> would output a link "999999" which would make a phone call to 999999. <code>[slash_phone number="999999" prefix="+30"]</code> would output a link "999999" which would make a phone call to +30999999. <code>[slash_phone number="999999" prefix="+30" text="Call us"]</code> would output a link "Call us" which would make a phone call to +30999999.', 'slash-admin' ),
			'type'    => 'checkbox',
			'std'     => 0
		);
		...
		$this->settings['shortcode_url'] = array( // Hide update notices for all but Admins
			...
			'desc'    => __( 'If you develop your site on localhost or on a temporary URL, you might want to avoid absolute URLs inside posts and pages. That way you don\'t need to update your links after migrating to your actual domain.  The available options are:<br />
					...
					Then, you can use them like so <code>&lt;img src="[slash_child]/images/image.png" /&gt;</code>, with which you would get <code>&lt;img src="http://www.yourdomain.com/wp-content/themes/childtheme/images/image.png" /&gt;</code>', 'slash-admin' ),
			'type'    => 'checkbox',
			'std'     => 0
		);
		...
		$this->settings['q0_heading'] = array(
			'section' => 'about',
			'title'   => '', // Not used for headings.
			'desc'    => __( 'My theme also supports some of this plugin\'s features. Which one should I choose?', 'slash-admin' ),
			'type'    => 'heading',
			'para'    => __( 'It is up to you to decide whether you will use your theme\'s options or those provided by this plugin. It is recommended, though, that you keep those settings separated from your theme and the reason is simple: If at some point you decide to switch themes, those options will be lost and you have to remember to re-enter them. Keeping them in a plugin maintains the options between themes.', 'slash-admin' )
		);
		...
		$this->settings['q3_heading'] = array(
			...
			'type'    => 'heading',
			'para'    => __( 'Generally speaking, no. Wordpress\' default behaviour is probably the best, that\'s why the specific option is disabled by default. In some cases, though, users might get confused with those notifications or think that something is wrong with the website. In cases like that, you might want to keep the update notifications visible only for those who can apply them - namely the administrators. Keep in mind that, technically, selecting this option won\'t remove the notifications for the non-admins - it will just hide them via CSS.', 'slash-admin' )
		);
		...
		$this->settings['q4_heading'] = array(
			...
			'type'    => 'heading',
			'para'    => __( 'You can allow editors access to one or more of the following sub-sections of the "Appearance" section: <br />- Customize<br />- Widgets<br />- Menus<br />- Background<br />Technically, by selecting even one of the above options you give editors access to the Appearance section. To prevent them from accessing unwanted subsections (e.g. you want them to see the Menus but not the Widgets) the plugin hides their links via CSS/JavaScript from both the backend and the frontend. If an editor knew the link for the Widgets subsection he/she could access it. By default the plugin respects the WordPress\' default behavior, keeping those options disabled (users have no access at all to the Appearance section).', 'slash-admin' )
		);
		...
		$this->settings['q6_heading'] = array(
			...
			'type'    => 'heading',
			'para'    => __( 'This plugin has been tested with Wordpress 4.0 and above. It might probably work with older versions too, but you should always use the latest version of Wordpress.', 'slash-admin' )
		);
		$this->settings['q7_heading'] = array(
			...
			'type'    => 'heading',
			'para'    => __( 'You can post a message at the plugin\'s <a target="_blank" href="http://wordpress.org/support/plugin/slash-admin">support forum</a> and I will to my best to help you out.', 'slash-admin' )
		);
	}
	...
	public function slashadmin_scripts() {
		...
		wp_enqueue_media();
		wp_register_script( 'slashadmin-upload', plugins_url( 'js/uploader.js', __FILE__ ), array(
			'jquery',
			'jquery-ui-tabs',
			'media-upload',
			'thickbox'
